 ...
 * OPL 22.1.1.0 Model
 * Author: Jakub
 * Creation Date: Apr 17, 2023 at 8:20:24 PM
...
aby największa odległość pomiędzy każdą wybraną lokalizacją dla obiektu a punktami
 ...
 *********************************************/
 ...
 minimize sum(i in l_r) (max(j in r_r) (x_b[i]*(abs(l_i[i][1]-r_i[j][1])) + (abs(l_i[i][2]-r_i[j][2])))); // zminimalizuj sumę minimów odległości
